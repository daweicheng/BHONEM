...
class LINE(object):
    ...
    # 在保存节点嵌入特征时，将网络中一同进行嵌入的担保人特征删去，仅保留客户的嵌入特征，继续之后的模型训练与预测
    def save_embeddings_deleted(self, filename1,delete_node):
        fout = open(filename1, 'w')
        ## delete the guaranteer node , only get the representation of custom node
        node_num = len(self.vectors.keys())
        fout.write("{} {}\n".format(node_num - len(delete_node), self.rep_size))

        for node, vec in self.vectors.items():
            #print(node not in node_det)
            if node not in delete_node:
                #print('not in')
                fout.write("{} {}\n".format(node,
                                            ' '.join([str(x) for x in vec])))
